     ...
     */
        ...
        displayname="Export" output="false" returnFormat="plain"
     {
        ...
        var summary = new Query( datasource = #MyDataSource#,
            sql = "select rl.Sort_Order, BID_UNIT, UOM, sum(QUANTITY) sq, avg(U_PRICE) u, sum( ISNULL ( QUANTITY * U_PRICE, 0 )) tot
                    from vwHDRAssessmentTracking a 
                    inner join vwUgly u on a.Location_No = u.Location_No
                    left join ReportLabel rl on u.DISPLAY_ORDER = rl.hdrk
                    where a.Package = :pnum AND QUANTITY > 0
                    group by BID_UNIT, UOM, rl.Sort_Order
                    order by ( case when rl.Sort_Order is null then 1 else 0 end), rl.Sort_Order, BID_UNIT" );
     ...
     {
        var q = new Query( datasource = MyDataSource,
            sql = "select rl.Sort_Order 'Item No', BID_UNIT, UOM, QUANTITY, U_PRICE, ISNULL( QUANTITY * U_PRICE, 0) 'Total'
                FROM vwUgly 
                left join ReportLabel rl on rl.hdrk = DISPLAY_ORDER
     ...
     {
        var q = new Query( datasource = MyDataSource,
            sql = "select rl.Sort_Order 'Item No', BID_UNIT, UOM, QUANTITY
                FROM vwUgly 
                left join ReportLabel rl on rl.hdrk = DISPLAY_ORDER
                WHERE Location_No = :loc AND Quantity > 0
                ORDER BY ( case when rl.Sort_Order is null then 1 else 0 end ), rl.Sort_Order, BID_UNIT");

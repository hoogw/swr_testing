...
from tblCurbRamps
where ( removed is null or removed = '' ) and construction_completed_date <> '' AND 
	Construction_Completed_Date >= <cfqueryparam value=#qs# CFSQLTYPE="CF_SQL_DATE" /> AND
...
where Construction_Completed_Date >= <cfqueryparam value=#qs# CFSQLTYPE="CF_SQL_DATE" />
and a.Category <> 'Non-SRP Construction'
<cfif qlock >
and ( a.locked = 'yes' )
</cfif>

...
<cfquery name="MyQuery" datasource="sidewalk">
	...
	WHERE     ( removed is null or removed = '' ) AND (c.Construction_Completed_Date <> '' AND c.Construction_Completed_Date >= '2017-7-1' AND
	c.Construction_Completed_Date <= <cfqueryparam value=#qe# CFSQLTYPE="CF_SQL_DATE" /> )
	<cfif qlock >
	and ( a.locked = 'yes' )
	</cfif>
	ORDER BY c.Construction_Completed_Date DESC, c.Location_No 
</cfquery>

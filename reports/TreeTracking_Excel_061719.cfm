...
<cfscript>
    ...
    select 11, 'Total Number of Trees Removed', count(*) from good  inner join vwHDRAssessmentTracking a on loc_no = a.Location_No where ACTION_TYPE = 'Removal' and TREE_REMOVAL_DATE is not null and t_type in ( 'BSS', 'RAP', 'General Service', 'BSS - Dead Tree', 'BSS - Volunteer' )
	union
    ...
    union
    select 31, 'Total Number of Trees Planted', count(*) from good  inner join vwHDRAssessmentTracking a on loc_no = a.Location_No where ACTION_TYPE = 'Planting' AND TREE_PLANTING_DATE is not null and t_type in ( 'BSS', 'RAP', 'General Service' )
    union
    ...
    SpreadsheetAddRow( myXls, "Total Number of Trees Assessed to be Root Pruned", 9,1); SpreadsheetSetCellValue( myXls, tpp.root_prune[1], 9,2);
    SpreadsheetAddRow( myXls, "Total Number of Trees Assessed to be Canopy Pruned", 10,1); SpreadsheetSetCellValue( myXls, tpp.canopy_prune[1], 10,2);
    ...
    totalQuery.setSql("with good ( package_no, ccdate, action_type, site_type, site_subtype, tree_planting_date, tree_removal_date, t_type, offsite) as
        ...
        select site_type, site_subtype
            , sum( case when action_type = 'Planting' and tree_planting_date is not null  and t_type in ( 'BSS', 'RAP', 'General Service' ) then 1 else 0 end ) planted
            , sum( case when action_type = 'Planting' and tree_planting_date is null and t_type in ( 'BSS', 'RAP', 'General Service' ) and ( package_no is not null ) and ( ccdate is not null ) then 1 else 0 end ) pplant
            , sum( case when action_type = 'Removal' and tree_removal_date is not null and t_type in ( 'BSS', 'RAP', 'General Service', 'BSS - Dead Tree', 'BSS - Volunteer' ) then 1 else 0 end ) removed
            , sum( case when action_type = 'Removal' and tree_removal_date is null and t_type in ( 'BSS', 'RAP', 'General Service', 'BSS - Dead Tree', 'BSS - Volunteer' ) and ( package_no is not null ) then 1 else 0 end ) premove
        from good
    ...
    totalQuery.setSql("with good ( package_no, ccdate, action_type, site_type, site_subtype, tree_planting_date, tree_removal_date, t_type, offsite) as
        ...
        select site_type, site_subtype, offsite
            , case when site_type = 'Rebate' then 'Rebate' else 'Sidewalk' end prgm
            , sum( case when action_type = 'Planting' and tree_planting_date is not null and offsite='no' and t_type in ( 'BSS', 'RAP', 'General Service' ) then 1 else 0 end ) planted
            , sum( case when action_type = 'Planting' and tree_planting_date is null and offsite='no'  and t_type in ( 'BSS', 'RAP', 'General Service' ) and ( package_no is not null ) and ( ccdate is not null ) then 1 else 0 end ) pplant
            , sum( case when action_type = 'Planting' and offsite='yes' and tree_planting_date is not null  and t_type in ( 'BSS', 'RAP', 'General Service' ) then 1 else 0 end ) offplanted
            , sum( case when action_type = 'Planting' and offsite='yes' and tree_planting_date is null and t_type in ( 'BSS', 'RAP', 'General Service' ) and ( package_no is not null ) and ( ccdate is not null ) then 1 else 0 end ) offpplant
            , sum( case when action_type = 'Removal' and tree_removal_date is not null and t_type in ( 'BSS', 'RAP', 'General Service', 'BSS - Dead Tree', 'BSS - Volunteer' ) then 1 else 0 end ) removed
            , sum( case when action_type = 'Removal' and tree_removal_date is null and t_type in ( 'BSS', 'RAP', 'General Service', 'BSS - Dead Tree', 'BSS - Volunteer' ) and ( package_no is not null ) then 1 else 0 end ) premove
        from good
    ...
    SpreadsheetAddRow( myXls, "Tree Removals, Trees Removed, 0,0", 6,1);
    SpreadsheetAddRow( myXls, "Tree Removals, Trees Pending Removal, 0,0", 7,1);
    SpreadsheetAddRow( myXls, "Tree Planted, Trees Planted - Onsite, 0,0", 8,1);
    SpreadsheetAddRow( myXls, "Tree Planted, Trees Planted - Offsite, 0,0", 9,1);
    SpreadsheetAddRow( myXls, "Pending Planting, Trees Pending Planting - Onsite, 0,0", 10,1);
    SpreadsheetAddRow( myXls, "Pending Planting, Trees Pending Planting - Offsite, 0,0", 11,1);
